...
### Local setup (without full monorepo install)

The `pnpm-lock.yaml` in this repo was generated by pnpm v9+; the current
environment may run pnpm v8, which cannot parse it. A full `pnpm install` in
this monorepo resolves all ~2400 packages (including React Native, Electron,
and Prisma) and can consume >2 GB of disk space.

To develop `pi-happy` locally, use the lightweight symlink script instead:

```bash
bash scripts/link-pi-happy-deps.sh
```

This installs the 7 runtime dependencies (`axios`, `commander`, `tweetnacl`,
`qrcode-terminal`, `socket.io-client`, `zod`, `@paralleldrive/cuid2`) into
`/tmp/pi-happy-deps` and symlinks them, along with the workspace packages
`happy-agent` and `@slopus/happy-wire`, into a minimal `node_modules` at the
repo root. Both workspace packages already have their `dist/` outputs checked
in, so no build step is required.

### Run tests

```bash
# All tests (unit + integration)
pnpm --filter pi-happy test

# Type checking
pnpm --filter pi-happy typecheck
```
